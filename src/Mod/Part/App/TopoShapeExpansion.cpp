...
Data::ElementMapPtr TopoShape::resetElementMap(Data::ElementMapPtr elementMap)
{
    ...
    if (elementMap) {
        ...
        // FC_WARN("sizec: " << _cache->cachedElementMap->size() << " size: " << elementMap->size());
...
void TopoShape::flushElementMap() const
{
    ...
    if (!elementMap(false) && this->_cache) {
        if (this->_cache->cachedElementMap) {
            ...
            // parent.setHistoryAlgorithm(getHistoryAlgorithm());
            ...
            self._cache = _cache;
            // self.setHistoryAlgorithm(getHistoryAlgorithm());
                        ...
                        && (generatedShapeKey.first.ShapeType() >= TopAbs_FACE && generatedShapeKey.first.ShapeType() < TopAbs_VERTEX)) 
                    {
                        ...
                        for (; xp.More(); xp.Next()) {
                            ...
                            if (strcmp(lowerElementTypeName, "Face") == 0) {
                                subshapeIndexName = Data::IndexedName::fromConst(lowerElementTypeName, faceInfo.find(foundSubshape));
                            } else if (strcmp(lowerElementTypeName, "Edge") == 0) {
                                subshapeIndexName = Data::IndexedName::fromConst(lowerElementTypeName, edgeInfo.find(foundSubshape));
                            } else if (strcmp(lowerElementTypeName, "Vertex") == 0) {
                                subshapeIndexName = Data::IndexedName::fromConst(lowerElementTypeName, vertexInfo.find(foundSubshape));
                            }
                            ...
                            if (subshapeIndexName) {
                                ...
                                if (subShapeName
                                    && std::find(linkedNames.begin(), linkedNames.end(), subShapeName) == linkedNames.end()
                                    && allGeneratedShapes.count(subshapeIndexName) == 0) 
                                {
...
)
{
    ...
    for (int index = 1; index <= freeCheck.NbClosedFreeBounds(); ++index) {
        ...
        // originalWire.setHistoryAlgorithm(getHistoryAlgorithm());
    ...
    // perimeterCompound.setHistoryAlgorithm(getHistoryAlgorithm());
...
)
{
    ...
    for (auto& face : shape.getSubTopoShapes(TopAbs_FACE)) {
        ...
        // outerWire.setHistoryAlgorithm(getHistoryAlgorithm());
        if (wires.empty()) {
        ...
        if (std::abs(innerOffset) > Precision::Confusion()) {
            ...
            // innerWires.setHistoryAlgorithm(getHistoryAlgorithm());
...
)
{
    ...
    if (shapesToProcess.size() > 0) {
        ...
        // offsetShape.setHistoryAlgorithm(getHistoryAlgorithm());
        ...
        if (wiresForMakingFaces.size() > 0) {
            ...
            // face.setHistoryAlgorithm(getHistoryAlgorithm());
...
TopoShape TopoShape::reverseEdge(const TopoShape& edge)
{
    ...
    res.setHistoryAlgorithm(edge.getHistoryAlgorithm());
...
)
{
    ...
    // tmp.setHistoryAlgorithm(getHistoryAlgorithm());
    if (op || (shape.Tag && shape.Tag != Tag)) {
        setShape(tmp._Shape);
        initCache();
        if (!Hasher) {
...
struct MapperPrism: MapperMaker
{
        ...
        : MapperMaker(maker)
    {
        ...
        if (shapes.size()) {
            ...
            TopoShape bottom;
            // bottom.setHistoryAlgorithm(getHistoryAlgorithm());
            ...
            // shape.setHistoryAlgorithm(getHistoryAlgorithm());
...
TopoShape& TopoShape::makeElementXor(const std::vector<TopoShape>& shapes, const char* op, double tol)
{
    ...
    for (size_t i = 1; i < inputs.size(); ++i) {
        ...
        // tempUnion.setHistoryAlgorithm(getHistoryAlgorithm());
        ...
        // tempCommon.setHistoryAlgorithm(getHistoryAlgorithm());
...
)
{
    ...
    for (;;) {
        try {
            result = base;
            result.Tag = Tag; // PartDesign features should not have the same tag as their BaseFeature. So, we set it to the tag of this shape.
...
)
{
    ...
    if (shape.getShape().ShapeType() == TopAbs_COMPOUND) {
        ...
        for (const auto& entry : shape.getSubTopoShapes()[0].getElementMap()) {
            FC_WARN("name: " << entry.name.toString() << " index: " << entry.index.toString());
        }
        FC_WARN("after print");
    }
    else {
