...
public:
    ...
    // This method searches through the element map to find a set of similar names for the input (`searchName`)
    // by utilizing the above doNamesMatch() function.
    std::vector<Data::MappedElement> findSimilarNames(const Data::MappedName &searchName) const override;

    static std::vector<Data::MappedElement> findSimilarNames(const Data::MappedName &searchName, const TopoShape &searchShape);
