...
// Ported from the OCCT implementation of ShapeFix_FaceConnect (removed
// upstream, see ShapeFix_FaceConnect.hxx for context).
                                          ...
                                          const double fixtoler)
{
    ...
    ***************************************************************/
    ...
    for (NCollection_DataMap<TopoDS_Shape, TopoDS_Shape, TopTools_ShapeMapHasher>::Iterator
             theFEIter(theFreeEdges);
         theFEIter.More();
         theFEIter.Next()) {
        // Get pair (face / free edge)
        theEdge = theFEIter.Key();
        theFace = theFEIter.Value();
                ...
                for (theConnectedListIter.Initialize(theConnectedList);
                    ...
                    if (myOriFreeEdges.IsBound(theSecondFace)) {
                        ...
                        if (theProcessed.IsBound(theSecondFace)) {
                            NCollection_List<TopoDS_Shape>& theProcCnxList =
                                theProcessed(theSecondFace);
                        ...
                        if (!skip_pair) {
                            ...
                            for (i = 1; i <= theNumOfFacesToSew; i++) {
                                ...
                                for (theOriginalIter.Initialize(myOriFreeEdges(theFaceToSew));
                                    ...
                                    for (theResultsIter.Initialize(
                                             myResFreeEdges(theOriginalIter.Value()));
                                         theResultsIter.More();
                                         theResultsIter.Next()) {
                            ...
                            if (sewing_ok) {
                                ...
                                for (i = 1; i <= theNumOfFacesToSew; i++) {
                                    for (theOriginalIter.Initialize(
                                             myOriFreeEdges(theFacesToSew(i)));
                                         theOriginalIter.More();
                                         theOriginalIter.Next()) {
                                        // Get original free edge
                                        theOrigE = theOriginalIter.Value();
                                        NCollection_List<TopoDS_Shape>& theOldFreeList =
                                            myResFreeEdges(theOrigE);
                                        ...
                                        while (theResultsIter.More()) {
                                            ...
                                            if (theSewer.IsModified(theAuxE)) {
                                                ...
                                                for (TopExp_Explorer expe(
                                                         theSewer.Modified(theAuxE),
                                                         TopAbs_EDGE);
                                                     expe.More();
                                                     expe.Next()) {
                                                    ...
                                                    if (theResultEdges.IsBound(theAuxE)) {
                                                        ...
                                                        myResSharEdges(theResultEdges(theAuxE))
                                                            .Append(theAuxE);
        ...
        ***************************************************************/
        ...
        for (NCollection_DataMap<TopoDS_Shape,
            ...
            for (theOriginalIter.Initialize(theOEIter.Value()); theOriginalIter.More();
                ...
                if (!emptywire) {
                    ...
                    dist1 = -1.;
                    dist2 = -1.;
                    for (TopExp_Explorer expv(theNewW, TopAbs_VERTEX); expv.More();
                         expv.Next()) {
                    ...
                    if (!theOldV1.IsSame(theNewV1)) {
                        if (theRepVertices.IsBound(theOldV1)) {
                            ...
                            bool found = false;
                            for (theIter1.Initialize(theList1); theIter1.More();
                                 theIter1.Next()) {
                    ...
                    if (!theOldV2.IsSame(theNewV2)) {
                        if (theRepVertices.IsBound(theOldV2)) {
                            ...
                            bool found = false;
                            for (theIter2.Initialize(theList2); theIter2.More();
                                 theIter2.Next()) {
        ...
        if (!theRepEdges.IsEmpty()) {
            ...
            for (NCollection_DataMap<TopoDS_Shape, TopoDS_Shape, TopTools_ShapeMapHasher>::
                     Iterator theREIter(theRepEdges);
                 theREIter.More();
                 theREIter.Next()) {
            ...
            if (!theReShape->Status(ShapeExtend_OK) && !theReShape->Status(ShapeExtend_FAIL1)) {
                ...
                if (!theRepVertices.IsEmpty()) {
                    ...
                    for (NCollection_DataMap<TopoDS_Shape,
                        ...
                        for (theN1Iter.Initialize(theRV1Iter.Value()); theN1Iter.More();
                             theN1Iter.Next()) {
                    ...
                    for (NCollection_DataMap<TopoDS_Shape,
                        ...
                        for (theN2Iter.Initialize(theRV2Iter.Value()); theN2Iter.More();
                             theN2Iter.Next()) {
                    ...
                    theReShape->Clear();
                    for (NCollection_DataMap<TopoDS_Shape, TopoDS_Shape, TopTools_ShapeMapHasher>::
                             Iterator theNVIter(theOldVertices);
                         theNVIter.More();
                         theNVIter.Next()) {
